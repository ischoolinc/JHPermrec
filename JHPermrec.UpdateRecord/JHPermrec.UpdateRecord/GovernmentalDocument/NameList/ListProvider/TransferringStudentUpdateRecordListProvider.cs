...
namespace JHPermrec.UpdateRecord.GovernmentalDocument.NameList
{
    class TransferringStudentUpdateRecordListProvider : INameListProvider
    {
        ...
        private string[] _CodeList;// = new string[] { "3" };


        public TransferringStudentUpdateRecordListProvider()
        {
            // 新竹轉入是包含復學6
            if (JHSchool.Permrec.Program.ModuleType == JHSchool.Permrec.Program.ModuleFlag.HsinChu)
                _CodeList = new string[] { "3","6"};
            else
                _CodeList = new string[] { "3" };
        ...
        public System.Xml.XmlElement CreateNameList(string schoolYear, string semester, List<XmlElement> list)
        {
            ...
            // 取得班級年級
            Dictionary<string, string> ClassGradeYear = DAL.DALTransfer.GetUpateRecordStudentGradeYearList(list);
            ...
            doc.LoadXml("<異動名冊 類別=\"轉入學生名冊\" 學年度=\"" + schoolYear + "\" 學期=\"" + semester + "\" 學校代號=\"" + GlobalOld.SchoolInformation.Code + "\" 學校名稱=\"" + GlobalOld.SchoolInformation.ChineseName + "\"/>");
            foreach (XmlElement var in list)
            {
                ...
                dataElement.SetAttribute("轉入轉出學校", helper.GetText("ContextInfo/ContextInfo/ImportExportSchool"));
                ...
                if(helper.GetText("UpdateCode")=="3")
                    dataElement.SetAttribute("異動類別", "轉入");

                if (helper.GetText("UpdateCode") == "6")
                    dataElement.SetAttribute("異動類別", "復學");

...
{
    class ContinueStudentUpdateRecordList:ReportBuilder 
    {
        // 續讀名冊
        ...
        {
            // 資料轉換
            Dictionary<string, StudBatchUpdateRecContentEntity> data = StudBatchUpdateRecEntity.ConvertGetContentData(source);
            ...
            template.Open(new MemoryStream(GDResources.JContinueStudentUpdateRecordListTemplate), FileFormatType.Excel2003);

            //要產生的excel檔
            Workbook wb = new Aspose.Cells.Workbook();
            wb.Open(new MemoryStream(GDResources.JContinueStudentUpdateRecordListTemplate), FileFormatType.Excel2003);
            ...
            wb.Worksheets[0].Cells[rowi, 4].PutValue(StudBatchUpdateRecEntity.GetContentSchoolName() + "  " + StudBatchUpdateRecEntity.GetContentSchoolYear()+ "學年第 " + StudBatchUpdateRecEntity.GetContentSemester() + "學期"+"                                                        列印日期：" + UpdateRecordUtil.ChangeDate1911(DateTime.Now.ToString()));;

...
namespace JHPermrec.UpdateRecord.Batch
{
    ...
    {        
        ...
        List<DAL.StudDiplomaInfoJuniorDiplomaNumber > studRecSDIs;
        List<DAL.StudDiplomaInfoJuniorDiplomaNumber> GraduateStudRecs;
        List<DAL.StudDiplomaInfoJuniorDiplomaNumber> NGraduateStudRecs;
        List<DAL.StudDiplomaInfoJuniorDiplomaNumber> StudDiplomaInfoJuniorDiplomaNumberList;
        ...
        // 班級座號排序
        private List<DAL.StudDiplomaInfoJuniorDiplomaNumber> SortClassSeatNo(List<DAL.StudDiplomaInfoJuniorDiplomaNumber> SDIRecList)
        {
            Dictionary<int, List<string>> tmpClassSort = new Dictionary<int, List<string>>();
            Dictionary<string, List<DAL.StudDiplomaInfoJuniorDiplomaNumber>> tmpSeatNoSort = new Dictionary<string, List<JHPermrec.UpdateRecord.DAL.StudDiplomaInfoJuniorDiplomaNumber>>();

            List<int> tmpNoSort = new List<int>();
            List<DAL.StudDiplomaInfoJuniorDiplomaNumber> tmpStudRecSDIs = new List<JHPermrec.UpdateRecord.DAL.StudDiplomaInfoJuniorDiplomaNumber>();


            foreach (DAL.StudDiplomaInfoJuniorDiplomaNumber sdi in SDIRecList)
            {
                int x = 0;
                int.TryParse(sdi.ClassDisplayOrder, out x);
                    ...
                    {
                        List<DAL.StudDiplomaInfoJuniorDiplomaNumber> tmp = new List<JHPermrec.UpdateRecord.DAL.StudDiplomaInfoJuniorDiplomaNumber> ();
                        tmp.Add(sdi);
            ...
            // 座號排序
            foreach (KeyValuePair<string, List<DAL.StudDiplomaInfoJuniorDiplomaNumber>> studRec in tmpSeatNoSort)
                studRec.Value.Sort(new Comparison<DAL.StudDiplomaInfoJuniorDiplomaNumber>(SeatNoCompare));
            ...
            foreach (int i in tmpNoSort)            
            {
                if (i > 0)
                    if (tmpClassSort.ContainsKey(i))
                        ...
                        {
                            foreach (DAL.StudDiplomaInfoJuniorDiplomaNumber sdi in tmpSeatNoSort[classid])
        ...
        private void btnSave_Click(object sender, EventArgs e)
        {
            ...
            if (chkInputData == true)
            {
                ...
                // 批示填入證書字號
                foreach (DAL.StudDiplomaInfoJuniorDiplomaNumber sdi in StudDiplomaInfoJuniorDiplomaNumberList)
        ...
        private void sortByStudNumber()
        {
            ...
            GraduateStudRecs.Sort(new Comparison<DAL.StudDiplomaInfoJuniorDiplomaNumber>(StudentNumberCompare));
            NGraduateStudRecs.Sort(new Comparison<DAL.StudDiplomaInfoJuniorDiplomaNumber>(StudentNumberCompare));
            string zeroStr = "";
            int noStart,noStart1;
            int.TryParse(txtGRNo.Text, out noStart);
            for (int i = 0; i < txtGRNo.Text.Length; i++)
                zeroStr += 0;
            // 畢業
            foreach (DAL.StudDiplomaInfoJuniorDiplomaNumber sdi in GraduateStudRecs)
            {
                string tmpStr = zeroStr + noStart;
                string tmpStrDocNo=txtGRDoc.Text + "字第" + tmpStr.Substring((tmpStr.Length - txtGRNo.Text.Length), txtGRNo.Text.Length) + "號";
            ...
            for (int i = 0; i < txtGDNo1.Text.Length; i++)
                zeroStr += 0;
            // 修業
            foreach (DAL.StudDiplomaInfoJuniorDiplomaNumber sdi in NGraduateStudRecs)
            {
                string tmpStr = zeroStr + noStart1;
                string tmpStrDocNo = txtGRDoc1.Text + "字第" + tmpStr.Substring((tmpStr.Length - txtGDNo1.Text.Length), txtGDNo1.Text.Length) + "號";
                sortGraduateDocNoDatas.Add(sdi.StudentID, tmpStrDocNo);
                noStart1++;
            }
        ...
        private void sortByClassSeatNo()
        {
            ...
            foreach (DAL.StudDiplomaInfoJuniorDiplomaNumber sdi in NGraduateStudRecs)
            {
                if (cboSortBySnum.Checked == true)
                {
                    if (!string.IsNullOrEmpty(sdi.StudentNumber))
                    {
                        string tmpStr = zeroStr + noStart1; 
                        string tmpStrDocNo = txtGRDoc1.Text + "字第" + tmpStr.Substring((tmpStr.Length - txtGDNo1.Text.Length), txtGDNo1.Text.Length) + "號";
                        sortGraduateDocNoDatas.Add(sdi.StudentID , tmpStrDocNo);
        ...
        private void getGRDocNoToDG()
        {
            ...
            StudDiplomaInfoJuniorDiplomaNumberList = DAL.DALTransfer.GetStudentDiplomaInfoJuniorDiplomaNumberListByGradeYear(cboGradeYear.Text);
            StudDiplomaInfoJuniorDiplomaNumberList.Sort(new Comparison<DAL.StudDiplomaInfoJuniorDiplomaNumber>(StudentNumberCompare));

            // 填入 DataGridView
            int dgRowIdx = 0, hasGDDocNoCount = 0, GraduateCount = 0, NGraduateCount = 0;
            foreach (DAL.StudDiplomaInfoJuniorDiplomaNumber  sdij in StudDiplomaInfoJuniorDiplomaNumberList)
            {
        ...
        {
            DAL.DALTransfer.SetStudentDiplomaInfoJuniorDiplomaNumberList(StudDiplomaInfoJuniorDiplomaNumberList);            
        }

...
{
    // 匯出轉入異動
    class ExportUpdateRecCode03 : SmartSchool.API.PlugIn.Export.Exporter
    {
        // 可勾選選項
        List<string> ExportItemList;

        public ExportUpdateRecCode03()
        {
            this.Image = null;
            this.Text = "匯出轉入異動";
        ...
        public override void InitializeExport(SmartSchool.API.PlugIn.Export.ExportWizard wizard)
        {
            ...
            wizard.ExportPackage += delegate(object sender, SmartSchool.API.PlugIn.Export.ExportPackageEventArgs e)
            {
                        ...
                        foreach (string field in e.ExportFields)
                        {
                            if (wizard.ExportableFields.Contains(field))
                            {
                                switch (field)
                                {
                                    ...
                                    case "轉入前學校": row.Add(field,sure.GetImportExportSchool()); break;
                                    ...
                                    case "異動類別": row.Add(field, "轉入"); break;
                                }
                            }
                        }
                        ExportCount++;
                ...
                prlp.SaveLog("學生.匯出異動", "匯出轉入異動", "共匯出轉入異動" + ExportCount + "筆資料.");

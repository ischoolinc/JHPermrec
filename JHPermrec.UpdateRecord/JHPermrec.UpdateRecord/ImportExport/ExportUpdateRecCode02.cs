...
{
    // 匯出畢業異動
    class ExportUpdateRecCode02:SmartSchool.API.PlugIn.Export.Exporter
    {
        // 可勾選選項
        List<string> ExportItemList;

        public ExportUpdateRecCode02()
        {
            this.Image = null;
            this.Text = "匯出畢業異動";
 ...
 //           ExportItemList.Add("異動類別");
        ...
        public override void InitializeExport(SmartSchool.API.PlugIn.Export.ExportWizard wizard)
        {
            ...
            { 
                Dictionary<string,List<DAL.StudUpdateRecordEntity>> StudUpdateRecordEntityListDic = DAL.DALTransfer2.GetStudListUpdateRecordEntityListByUpdateType(e.List ,JHPermrec.UpdateRecord.DAL.DALTransfer2.UpdateType.畢業);
                int ExportCount = 0;
                        ...
                        foreach (string field in e.ExportFields)
                        {
                            if (wizard.ExportableFields.Contains(field))
                            {
                                switch (field)
                                {
                                    ...
                                    case "畢業年月": row.Add(field, sure.GetGraduateSchoolYear()); break;
                                    case "畢修業別": row.Add(field, sure.GetGraduate()); break;
                                    ...
                                    case "畢(結)業證書字號": row.Add(field, sure.GetGraduateCertificateNumber()); break;
                                    ...
                                    case "異動出生地": row.Add(field, sure.GetBirthPlace()); break;
...
//                                    case "異動類別": row.Add(field, "畢業"); break;
                                }
                            }
                        }
                        ExportCount++;
                ...
                prlp.SaveLog("學生.匯出異動", "匯出畢業異動", "共匯出畢業異動" + ExportCount + "筆資料.");
            };
        }
    }
}

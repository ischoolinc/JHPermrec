...
{
    // 匯出轉出異動
    class ExportUpdateRecCode04 : SmartSchool.API.PlugIn.Export.Exporter
    {
        // 可勾選選項
        List<string> ExportItemList;

        public ExportUpdateRecCode04()
        {
            this.Image = null;
            this.Text = "匯出轉出異動";
        ...
        public override void InitializeExport(SmartSchool.API.PlugIn.Export.ExportWizard wizard)
        {
            ...
            {
                Dictionary<string, List<DAL.StudUpdateRecordEntity>> StudUpdateRecordEntityListDic = DAL.DALTransfer2.GetStudListUpdateRecordEntityListByUpdateType(e.List, JHPermrec.UpdateRecord.DAL.DALTransfer2.UpdateType.轉出);
                int ExportCount = 0;
                        ...
                        foreach (string field in e.ExportFields)
                        {
                            if (wizard.ExportableFields.Contains(field))
                            {
                                switch (field)
                                {
                                    ...
                                    case "轉出後學校": row.Add(field, sure.GetImportExportSchool()); break;
                                    ...
                                    case "異動類別": row.Add(field, "轉出"); break;
                                }
                            }
                        }
                        ExportCount++;
                ...
                prlp.SaveLog("學生.匯出異動", "匯出轉出異動", "共匯出轉出異動" + ExportCount + "筆資料.");

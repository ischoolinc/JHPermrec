...
{
    // 匯出更正學籍異動
    class ExportUpdateRecCode09:SmartSchool.API.PlugIn.Export.Exporter
    {
        // 可勾選選項
        List<string> ExportItemList;

        public ExportUpdateRecCode09()
        {
            this.Image = null;
            this.Text = "匯出更正學籍異動";
        ...
        public override void InitializeExport(SmartSchool.API.PlugIn.Export.ExportWizard wizard)
        {
            ...
            {
                Dictionary<string, List<DAL.StudUpdateRecordEntity>> StudUpdateRecordEntityListDic = DAL.DALTransfer2.GetStudListUpdateRecordEntityListByUpdateType(e.List, JHPermrec.UpdateRecord.DAL.DALTransfer2.UpdateType.更正學籍);
                int ExportCount = 0;
                        ...
                        foreach (string field in e.ExportFields)
                        {
                            if (wizard.ExportableFields.Contains(field))
                            {
                                switch (field)
                                {
                                    ...
                                    case "異動類別": row.Add(field, "更正學籍"); break;
                                }
                            }
                        }
                        ExportCount++;
                ...
                prlp.SaveLog("學生.匯出異動", "匯出更正學籍異動", "共匯出更正學籍異動" + ExportCount + "筆資料.");
            };
        }
    }
}

...
{
    // 匯出復學
    class ExportUpdateRecCode06: SmartSchool.API.PlugIn.Export.Exporter 
    {
        // 可勾選選項
        List<string> ExportItemList;

        public ExportUpdateRecCode06()
        {
            this.Image = null;
            this.Text = "匯出復學異動";
        ...
        public override void InitializeExport(SmartSchool.API.PlugIn.Export.ExportWizard wizard)
        {
            ...
            {
                Dictionary<string, List<DAL.StudUpdateRecordEntity>> StudUpdateRecordEntityListDic = DAL.DALTransfer2.GetStudListUpdateRecordEntityListByUpdateType(e.List, JHPermrec.UpdateRecord.DAL.DALTransfer2.UpdateType.復學);
                int ExportCount = 0;
                        ...
                        foreach (string field in e.ExportFields)
                        {
                            if (wizard.ExportableFields.Contains(field))
                            {
                                switch (field)
                                {
                                    ...
                                    case "異動類別": row.Add(field, "復學"); break;
                                }
                            }
                        }
                        ExportCount++;
                ...
                prlp.SaveLog("學生.匯出異動", "匯出復學異動", "共匯出復學異動" + ExportCount + "筆資料.");
            };
        }
    }
}

...
{
    class ImportUpdateRecCode04 : SmartSchool.API.PlugIn.Import.Importer
    {
        public ImportUpdateRecCode04()
        {
            this.Image = null;
            this.Text = "匯入轉出異動";
        }
        ...
        public override void InitializeImport(SmartSchool.API.PlugIn.Import.ImportWizard wizard)
        {
            ...
            wizard.ImportableFields.AddRange("學年度", "學期", "異動年級", "異動日期", "備註", "學籍核准日期", "學籍核准文號", "異動班級", "異動姓名", "異動身分證號", "異動地址", "異動學號", "異動性別", "異動生日", "異動座號", "核准日期", "核准文號", "轉入學校", "異動原因", "異動類別", "轉出後學校", "原因及事項");
                    ...
                    switch (field)
                    {
                        ...
                        case "異動類別":
                            if (value != "轉出")
                            {
                                ...
                                e.ErrorFields.Add(field, "必須填入轉出");
                            }
                            break;
            ...
            wizard.ImportPackage += delegate(object sender, SmartSchool.API.PlugIn.Import.ImportPackageEventArgs e)
            {
                    ...
                    foreach (RowData data in id_Rows[id])
                    {
                        ...
                        foreach (string field in e.ImportFields)
                        {
                            ...
                            switch (field)
                            {
                                ...
                                case "轉入學校":
                                    updateRec.ImportExportSchool = value;
                                    break;
                ...
                try
                {
                    ...
                    prlp.SaveLog("學生.匯入異動", "匯入轉出異動", "匯入轉出異動：共新增" + InsertList.Count + "筆資料,共更新:" + UpdateList.Count + "筆資料");

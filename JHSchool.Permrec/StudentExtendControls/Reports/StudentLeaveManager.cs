...
{
    class StudentLeaveManager
    {

        List<DAL.StudentLeaveEntity> StudentLeaveEntityList;
        ...
        public StudentLeaveManager()
        {
            StudentLeaveEntityList = new List<JHSchool.Permrec.StudentExtendControls.Reports.DAL.StudentLeaveEntity>();
            GetUserDefineTemplateFromSystem();

            // 判斷使用高雄樣版
            if (Program.ModuleType == Program.ModuleFlag.KaoHsiung)
                defalutTemplate = JHSchool.Permrec.StudentExtendControls.Reports.RptResource.高雄休學證明書樣版;
        ...
        public void GetUserDefineTemplateFromSystem()
        {
            ...
            School.Configuration.Sync("休學證明書_無成績");
            ...
            if (ofd.ShowDialog() == DialogResult.OK)
            {
                try
                {
                    if (FileFormatUtil.DetectFileFormat(ofd.FileName).LoadFormat == LoadFormat.Doc)
                    {
                        ...
                        prlp.SaveLog("學生.報表", "上傳", "上傳休學證明書樣版.");
        ...
        {
            GetUserDefineTemplateFromSystem();
            StudentLeaveEntityList = DAL.DALTransfer.GetStudentLeaveEntityList(StudentIDList);
        ...
        void bkWorkPrint_DoWork(object sender, DoWorkEventArgs e)
        {
            ...
            foreach (DAL.StudentLeaveEntity se in StudentLeaveEntityList)
            {
                ...
                baseData.Add("民國休學異動日期", se.LeaveUpdateDateC);
                baseData.Add("民國休學結束日期", se.LeaveEndDateC);
                baseData.Add("休學期間", se.LeavePeriodString);
                baseData.Add("證書字號", se.CertDocNo);
                ...
                bkWorkPrint.ReportProgress((int)(((double)cot++ * 100.0) / (double)StudentLeaveEntityList.Count));
            }
            e.Result = doc;
        }

...
namespace Student_JSBT_HsinChu101
{
    class StudSBTManager
    {
        ...
        public void ExportDataToExcel(List<StudInfoEntity> StudInfoEntityList)
        {
            ...
            ColumnIdx.Add("資料授權", 21);
            ColumnIdx.Add("家長姓名", 22);
            ColumnIdx.Add("緊急連絡電話", 23);
            ColumnIdx.Add("郵遞區號", 24);
            ColumnIdx.Add("地址", 25);
            ColumnIdx.Add("手機", 26);
            ...
            try
            {
                ...
                wb.Save(Application.StartupPath + "\\Reports\\產生竹苗區免試入學報名檔.xls", FileFormatType.Excel2003);
                System.Diagnostics.Process.Start(Application.StartupPath + "\\Reports\\產生竹苗區免試入學報名檔.xls");                
            }
            catch
            {
                System.Windows.Forms.SaveFileDialog sd1 = new System.Windows.Forms.SaveFileDialog();
                sd1.Title = "另存新檔";
                sd1.FileName = "產生竹苗區免試入學報名檔.xls";
